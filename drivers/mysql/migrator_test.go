...
func TestMySQLMigrator_NewMySQLMigrator(t *testing.T) {
	...
	_, ok := migrator.(*MySQLMigratorWrapper)
	require.True(t, ok)
}
...
func TestMySQLMigrator_GetDatabaseType(t *testing.T) {
	...
	migrator := db.GetMigrator()
	assert.Equal(t, "mysql", migrator.GetDatabaseType())
}
...
func TestMySQLMigrator_GetTableInfo(t *testing.T) {
	...
	assert.Equal(t, "1", activeCol.Default) // MySQL returns "1" for boolean TRUE
...
func TestMySQLMigrator_GenerateModifyColumnSQL(t *testing.T) {
	...
	t.Run("Add UNIQUE constraint to column", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.Len(t, sqls, 1)
		assert.Contains(t, sqls[0], "ALTER TABLE `users` MODIFY COLUMN")
		assert.Contains(t, sqls[0], "`email` VARCHAR(255)")
	...
	t.Run("Change column type", func(t *testing.T) {
		...
		assert.Len(t, sqls, 1)
		assert.Contains(t, sqls[0], "ALTER TABLE `users` MODIFY COLUMN")
		assert.Contains(t, sqls[0], "`age` BIGINT")
	})
	...
	t.Run("Rename column", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.Len(t, sqls, 1)
		assert.Contains(t, sqls[0], "ALTER TABLE `users` CHANGE COLUMN")
		assert.Contains(t, sqls[0], "`name`")
		assert.Contains(t, sqls[0], "`full_name` VARCHAR(100)")
	})
}
...
func TestMySQLMigrator_GenerateModifyColumnSQL_Integration(t *testing.T) {
	...
	require.NoError(t, err)
	_, err = db.Exec("INSERT INTO test_products (name, price, description) VALUES ('Gadget', 200, 'A cool gadget')")
	require.NoError(t, err)
...
func TestMySQLMigrator_IndexComparison(t *testing.T) {
	skipIfMySQLNotAvailable(t)
...
func TestMySQLMigrator_SystemIndexDetection(t *testing.T) {
	skipIfMySQLNotAvailable(t)
	...
	// Create MySQL-specific migrator
	mysqlDB := db

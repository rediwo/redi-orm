...
func TestMySQLRawQuery_Exec(t *testing.T) {
	...
	t.Run("INSERT", func(t *testing.T) {
		query := db.Raw("INSERT INTO test_raw_query (name, value) VALUES (?, ?)", "test1", 100)
	...
	t.Run("UPDATE", func(t *testing.T) {
		query := db.Raw("UPDATE test_raw_query SET value = ? WHERE name = ?", 200, "test1")
		result, err := query.Exec(ctx)
		assert.NoError(t, err)
		assert.Equal(t, int64(1), result.RowsAffected)
	})
	...
	t.Run("DELETE", func(t *testing.T) {
		query := db.Raw("DELETE FROM test_raw_query WHERE name = ?", "test1")
		result, err := query.Exec(ctx)
		assert.NoError(t, err)
		assert.Equal(t, int64(1), result.RowsAffected)
	})
...
func TestMySQLRawQuery_Find(t *testing.T) {
	...
	// Insert test data
	_, err = db.Exec("INSERT INTO test_find (name, value) VALUES ('test1', 100), ('test2', 200), ('test3', 300)")
	require.NoError(t, err)
	...
	t.Run("Find with pointers", func(t *testing.T) {
		...
		var results []*TestRow
		query := db.Raw("SELECT id, name, value FROM test_find WHERE value >= ? ORDER BY id", 200)
		err := query.Find(ctx, &results)
		assert.NoError(t, err)
...
func TestMySQLRawQuery_FindOne(t *testing.T) {
	...
	// Insert test data
	_, err = db.Exec("INSERT INTO test_find_one (id, name) VALUES (1, 'test1')")
	require.NoError(t, err)
	...
	t.Run("Find single row struct", func(t *testing.T) {
		...
		var result TestRow
		query := db.Raw("SELECT id, name FROM test_find_one WHERE id = ?", 1)
	...
	t.Run("Find single value", func(t *testing.T) {
		var name string
		query := db.Raw("SELECT name FROM test_find_one WHERE id = ?", 1)
	...
	t.Run("No rows found", func(t *testing.T) {
		var result string
		query := db.Raw("SELECT name FROM test_find_one WHERE id = ?", 999)
		err := query.FindOne(ctx, &result)
		assert.Error(t, err)
	})
}
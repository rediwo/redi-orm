...
func TestQueryBuilder(t *testing.T) {
	...
	t.Run("Simple select", func(t *testing.T) {
		qb := db.Select("users", []string{"name", "email"})
	...
	t.Run("Where clause", func(t *testing.T) {
		qb := db.Select("users", nil).Where("age", "=", 25)
		results, err := qb.Execute()
		if err != nil {
	...
	t.Run("WhereIn", func(t *testing.T) {
		qb := db.Select("users", nil).WhereIn("name", []interface{}{"Alice", "Bob"})
		results, err := qb.Execute()
		if err != nil {
	...
	t.Run("OrderBy", func(t *testing.T) {
		qb := db.Select("users", []string{"name"}).OrderBy("name", "DESC")
		results, err := qb.Execute()
		if err != nil {
	...
	t.Run("Limit and Offset", func(t *testing.T) {
		qb := db.Select("users", nil).OrderBy("name", "ASC").Limit(2).Offset(1)
		results, err := qb.Execute()
		if err != nil {
	...
	t.Run("First", func(t *testing.T) {
		qb := db.Select("users", nil).Where("age", "=", 30)
		result, err := qb.First()
		if err != nil {
		...
		qb2 := db.Select("users", nil).Where("age", "=", 99)
		_, err = qb2.First()
		if err == nil {
	...
	t.Run("Count", func(t *testing.T) {
		...
		qb2 := db.Select("users", nil).Where("age", ">=", 30)
	...
	t.Run("Complex query", func(t *testing.T) {
		qb := db.Select("users", []string{"name", "age"}).
			Where("age", ">=", 25).
			Where("age", "<=", 30).
			OrderBy("age", "ASC").
			Limit(10)
